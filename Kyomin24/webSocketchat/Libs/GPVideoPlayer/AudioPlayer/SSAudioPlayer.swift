...
//  World Album
//
//  Created by Shubham Sharma on 09/04/20.
...
protocol SSAudioPlayerDelegate {
	func stopPlayer()
}
class SSAudioPlayer: UIView {
...
//        NotificationCenter.default.removeObserver(self)
	...
	//MARK: Public Methods
	public class func initialize(with frame: CGRect) -> SSAudioPlayer? {
		...
		let view = UINib(nibName: Constants.nibName, bundle: nil).instantiate(withOwner: self, options: nil).first as? SSAudioPlayer
			
		//	Bundle.main.loadNibNamed(Constants.nibName, owner: self, options: nil)?.first as? SSAudioPlayer
...
private extension SSAudioPlayer {
